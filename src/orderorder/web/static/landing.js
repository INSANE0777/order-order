// OrderOrder — Landing Page Script
...
(() => {
  ...
  // =========================================================================
  // INTERACTIVE ASCII BRANDMARK FOOTER SHOWPIECE
  // Multi-phase state machine: logo -> scattered -> fallen -> returning -> logo
  // Cursor repel physics with damping & restitution bounce under gravity
  // =========================================================================
  function initAsciiLogo() {
    const stage = $("asciiStage");
    ...
    const CELL_SIZE = 10;
    const CELL_GAP = 1;
    const CELL_STEP = CELL_SIZE + CELL_GAP; // 11px
    const ASCII_COLOR = "#f3ece5"; // Signature crisp cream on black
    const BRIGHTNESS_THRESHOLD = 0.16;
    const MIN_WIDTH = 900;

    const HOVER_RADIUS = 7.5; // in grid cells
    const HOVER_PUSH = 3.2;
    ...
    const RESET_EASE = 0.075;
    const STAGGER_FRAMES = 18;

    // Rich ASCII glyph pool matching Awwwards reference
    const GLYPHS = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789§¶#*@%";

    let dpr = Math.min(window.devicePixelRatio || 1, 2);
    let phase = "logo"; // "logo" | "scattered" | "fallen" | "returning"
    let cursorX = -999;
    let cursorY = -999;
    ...
    const easeToward = (cell, tx, ty, ease) => {
    ...
    const staggerCells = () => {
      cells.forEach((cell) => {
        cell.delay = Math.floor(Math.random() * STAGGER_FRAMES);
      });
    };
    ...
    function buildAsciiGrid() {
      ...
      if (width < MIN_WIDTH || height < 100) {
        cells = [];
        return;
      }

      gridCols = Math.max(1, Math.floor(width / CELL_STEP));
      gridRows = Math.max(1, Math.floor(height / CELL_STEP));
      ...
      // Offscreen sampling canvas (1 pixel = 1 cell)
      ...
      sctx.fillRect(0, 0, gridCols, gridRows);

      // Draw giant bold uppercase wordmark with space between each letter: O R D E R   O R D E R
      const word = "O R D E R   O R D E R";
      let fontSize = Math.floor(gridRows * 0.55);
      sctx.font = `900 ${fontSize}px "Geist", -apple-system, sans-serif`;
      let metrics = sctx.measureText(word);
      const targetW = gridCols * 0.92;
      if (metrics.width > 0) {
        fontSize = Math.floor(fontSize * (targetW / metrics.width));
        fontSize = Math.min(fontSize, Math.floor(gridRows * 0.65));
      }
      sctx.font = `900 ${fontSize}px "Geist", -apple-system, sans-serif`;
      sctx.textAlign = "center";
      sctx.textBaseline = "middle";
      sctx.fillStyle = "#ffffff";
      sctx.fillText(word, gridCols / 2, gridRows / 2);

      // Sample pixels
      const imgData = sctx.getImageData(0, 0, gridCols, gridRows).data;
      const lit = new Set();
      ...
      lit.forEach((key) => {
        ...
        cells.push({
          ...
          x: col,
          y: row,
          scatterX: col + (Math.random() * 2 - 1) * SCATTER_RANGE,
          scatterY: row + (Math.random() * 2 - 1) * SCATTER_RANGE,
          fallSpeed: 0,
          delay: 0,
        });
      });
    ...
    function updateAscii() {
      ...
      for (let i = 0; i < cells.length; i++) {
        ...
        if (phase === "scattered") {
          ...
          const floor = gridRows - 1;
          cell.fallSpeed += GRAVITY;
          cell.y += cell.fallSpeed;

          if (cell.y >= floor) {
            cell.y = floor;
            cell.fallSpeed *= -BOUNCE;
            if (Math.abs(cell.fallSpeed) < 0.035) {
              cell.fallSpeed = 0;
            }
          }
        } else if (phase === "returning") {
          easeToward(cell, cell.goalX, cell.goalY, RESET_EASE);
          const dist = Math.hypot(cell.goalX - cell.x, cell.goalY - cell.y);
          if (dist > 0.08) {
            everyoneHome = false;
          }
        } else {
          // Resting "logo" phase with cursor repel
          const dx = cell.x - cursorX;
          const dy = cell.y - cursorY;
          const dist = Math.hypot(dx, dy);

          let targetX = cell.goalX;
          let targetY = cell.goalY;

          if (dist < HOVER_RADIUS) {
            const strength = (1 - dist / HOVER_RADIUS) * HOVER_PUSH;
            const nx = dist > 0.001 ? dx / dist : 0;
            const ny = dist > 0.001 ? dy / dist : 0;
            targetX += nx * strength;
            targetY += ny * strength;

            // Subtle ASCII character shimmer on cursor proximity
            if (Math.random() < 0.12) {
              cell.glyph = randGlyph();
            }
          }
      ...
      if (phase === "returning" && everyoneHome) {
        phase = "logo";
      }
    }
    ...
    function drawAscii() {
      ...
      ctx.font = `${CELL_SIZE}px "Geist Mono", "Courier New", monospace`;
      ...
      for (let i = 0; i < cells.length; i++) {
        const c = cells[i];
        const px = c.x * CELL_STEP + CELL_SIZE / 2;
        const py = c.y * CELL_STEP;
    ...
    stage.addEventListener("pointermove", (e) => {
      ...
      cursorX = (e.clientX - r.left) / CELL_STEP;
      cursorY = (e.clientY - r.top) / CELL_STEP;
    });
    ...
    stage.addEventListener("pointerdown", () => {
      ...
      if (phase === "logo") {
        phase = "scattered";
        cells.forEach((c) => {
          c.scatterX = c.col + (Math.random() * 2 - 1) * SCATTER_RANGE;
          c.scatterY = c.row + (Math.random() * 2 - 1) * SCATTER_RANGE;
        });
        staggerCells();
        ...
        cells.forEach((c) => {
          c.fallSpeed = 0;
          c.delay = Math.floor(Math.random() * 6);
        });
      } else if (phase === "fallen") {
        phase = "returning";
        staggerCells();
    ...
    if (document.fonts) {
      document.fonts.ready.then(() => buildAsciiGrid());
    }
